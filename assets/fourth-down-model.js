...
function interpolateExpectedPoints(yardLine) {
  ...
  return expectedPointsAnchors[expectedPointsAnchors.length - 1].points;
}
...
function getFieldGoalSuccessRate(distance) {
  if (distance <= 27) return 0.99;
  if (distance <= 32) return 0.97;
  if (distance <= 37) return 0.94;
  if (distance <= 42) return 0.88;
  if (distance <= 47) return 0.79;
  if (distance <= 52) return 0.66;
...
function buildExplanation(input, recommendation, conversionProbability, adjustments, fieldGoalDistance) {
  ...
  if (recommendation === "Go for It") {
    ...
    return fieldPositionNote + " A " + Math.round(conversionProbability * 100) +
      "% conversion estimate keeps going-for-it competitive. " + urgencyNote;
  }
  ...
  if (recommendation === "Field Goal") {
    ...
    return fieldPositionNote + " From roughly " + fieldGoalDistance +
      " yards, the field goal remains credible enough to beat the alternatives. " + leverageNote;
  }
...
function evaluateFourthDownDecision(rawInput) {
  ...
  const options = [
    { label: "Go for It", expectedValue: goExpectedValue },
    { label: "Punt", expectedValue: puntExpectedValue },
    { label: "Field Goal", expectedValue: fieldGoalExpectedValue }
  ];
